...
contract TesserAssociativaSBTv2 is ERC721, Ownable {
    struct Ticket {
        bool exists;
        string name;
        string description;
    ...
    mapping(address => uint256) public addressTicket;
    ...
    function mint(
        ...
        for (uint256 k = 0; k < amount; k++) {
            ...
            addressTicket[receiver] = tokenId;
            emit Minted(tokenId);
        }
    }
    ...
    function burn(uint256 tokenId) public {
        ...
        addressTicket[owner] = 0;
        _burn(tokenId);
        validUntil[tokenId] = 0;
        emit Burned(tokenId);
    }
    ...
    function _afterTokenTransfer(
        ...
        addressTicket[from] = 0;
        addressTicket[to] = tokenId;
    }

    function balanceOf(address owner) public view virtual override returns (uint256) {
        require(owner != address(0), "ERC721: balance query for the zero address");
        uint256 tokenId = addressTicket[owner];
        if (validUntil[tokenId] < block.timestamp) {
            return 0;
        }
        return 1;
    }
